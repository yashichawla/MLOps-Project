...
def salad_ml_evaluation_pipeline_v1():
    ...
    dvc_pull = BashOperator(
        ...
        bash_command=f'''
            ...
            # 2.5) Clean up any leftover .gstmp files from interrupted GCS operations
            # These temporary files can block DVC checkout operations
            echo "[info] Cleaning up any leftover .gstmp temporary files from GCS..."
            find data/ -name "*.gstmp" -type f -delete 2>/dev/null || true
            find data/ -name "*_.gstmp" -type f -delete 2>/dev/null || true
            ...
            CHECKOUT_OUTPUT=$(python -m dvc checkout salad_preprocess -f 2>&1)
            CHECKOUT_EXIT=$?
            set -e  # Re-enable exit on error
            echo "$CHECKOUT_OUTPUT"
            if [ $CHECKOUT_EXIT -ne 0 ]; then
                echo "[warn] dvc checkout had some errors (exit code: $CHECKOUT_EXIT), but continuing..."
                echo "[warn] This may be OK if outputs don't exist in remote yet or files are locked"
            else
                echo "[info] dvc checkout completed successfully"
            fi
            
            echo "$PULL_OUTPUT"
            
            # Check if error is due to .gstmp files (GCS temporary files), confirmation errors, or missing outputs
            if [ $PULL_EXIT -ne 0 ]; then
                if echo "$PULL_OUTPUT" | grep -qi "\.gstmp.*confirmation\|unable to remove.*\.gstmp"; then
                    echo "[warn] Found .gstmp temporary files blocking DVC pull, cleaning up and retrying..."
                    find data/ -name "*.gstmp" -type f -delete 2>/dev/null || true
                    find data/ -name "*_.gstmp" -type f -delete 2>/dev/null || true
                    echo "[info] Retrying dvc pull after cleanup..."
                    set +e
                    PULL_OUTPUT=$(python -m dvc pull -v 2>&1)
                    PULL_EXIT=$?
                    set -e
                    echo "$PULL_OUTPUT"
                    if [ $PULL_EXIT -ne 0 ]; then
                        echo "[warn] DVC pull still had issues after cleanup, but continuing..."
                    else
                        echo "[info] DVC pull succeeded after cleanup"
                    fi
                elif echo "$PULL_OUTPUT" | grep -qi "unable to remove.*without a confirmation"; then
                    echo "[warn] Found confirmation error - DVC needs to remove conflicting files"
                    echo "[warn] This happens when local files differ from dvc.lock"
                    # Extract file path from error message
                    CONFLICTING_FILE=$(echo "$PULL_OUTPUT" | sed -n "s/.*unable to remove '\([^']*\)' without a confirmation.*/\1/p" | head -1)
                    if [ -n "$CONFLICTING_FILE" ] && [ -f "$CONFLICTING_FILE" ]; then
                        echo "[warn] Conflicting file: $CONFLICTING_FILE"
                        # Check if it's a model output that will be regenerated anyway
                        if echo "$CONFLICTING_FILE" | grep -qE "(data/responses|data/judge|data/bias|data/metrics/additional)"; then
                            echo "[info] File is a model output - will be regenerated by DAG if needed"
                            echo "[info] Removing conflicting file to allow dvc pull to proceed..."
                            rm -f "$CONFLICTING_FILE" || true
                        else
                            echo "[warn] File is not a model output - removing to allow dvc pull to proceed..."
                            rm -f "$CONFLICTING_FILE" || true
                        fi
                        echo "[info] Retrying dvc pull after removing conflicting file..."
                        set +e
                        PULL_OUTPUT=$(python -m dvc pull -v 2>&1)
                        PULL_EXIT=$?
                        set -e
                        echo "$PULL_OUTPUT"
                        if [ $PULL_EXIT -ne 0 ]; then
                            echo "[warn] DVC pull still had issues after removing conflicting file, but continuing..."
                        else
                            echo "[info] DVC pull succeeded after removing conflicting file"
                        fi
                    else
                        echo "[warn] Could not extract file path from error, retrying with force flag..."
                        set +e
                        PULL_OUTPUT=$(python -m dvc pull -f -v 2>&1)
                        PULL_EXIT=$?
                        set -e
                        echo "$PULL_OUTPUT"
                        if [ $PULL_EXIT -ne 0 ]; then
                            echo "[warn] DVC pull still had issues with force flag, but continuing..."
                        else
                            echo "[info] DVC pull succeeded with force flag"
                        fi
                    fi
                elif echo "$PULL_OUTPUT" | grep -qi "failed to restore\|missing\|not found"; then
